...
module.exports = function( Release ) {
	Release.define({
		...
		_getNpmInfo: function( npmPackage ) {
			var result = shell.exec( "npm info " + npmPackage, { silent: true } );
			if ( result.code !== 0 ) {
				Release.abort( "Error getting npm info." );
			}
		...
		_shrinkwrap: function() {
			if ( !Release.npmShrinkwrap ) {
				return;
			}

			Release.chdir( Release.dir.repo );

			var npmCommand = "npm prune";
			console.log( "  " + chalk.cyan( npmCommand ) );
			Release.exec( npmCommand );

			npmCommand = "npm shrinkwrap";
			console.log( "  " + chalk.cyan( npmCommand ) );
			Release.exec( npmCommand );

			console.log( "Adding npm-shrinkwrap.json..." );
			Release.exec( "git add npm-shrinkwrap.json", "Error adding npm-shrinkwrap.json to git." );
